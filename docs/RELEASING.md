...
1. Update `package.json`, lockfile, the quickstart tarball filename and changelog.
2. Run `npm run typecheck`, `npm test`, `npm run build`, `npm run test:e2e` and `npm run test:package` with Chromium installed.
...
6. Verify the public install and add release notes. Update installation docs from local-tarball instructions after the first publication. Prereleases use `next`; stable releases use `latest`.
